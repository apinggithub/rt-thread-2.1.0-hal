...
#ifndef USART_WIFI_H_INCLUDED
#define USART_WIFI_H_INCLUDED

#include <stdint.h>
#include <rtdef.h>
// little-endian
struct usart_cmd_request
{
    uint32_t flag;
...
/* usart slave configure. */
#define USART_MAX_DATA_LEN    1520
#define USART_TX_POOL_SIZE    2
#define USART_RX_POOL_SIZE    2
...
struct usart_data_packet
{
    uint32_t data_len;
    uint32_t data_type;
    char buffer[USART_MAX_DATA_LEN];
};

/********************************* ESP12F **************************************/

/* option */
#define ESP12F_CMD_TIMEOUT           (RT_TICK_PER_SECOND*3)
...
typedef struct _esp12f_ap_info
{
...
typedef struct _esp12f_cmd_init
{
    uint32_t mode;
} esp12f_cmd_init;

typedef struct _esp12f_resp_init
{
...
typedef struct _esp12f_cmd_easy_join
{
    char ssid[SSID_NAME_LENGTH_MAX];
    char passwd[PASSWORD_LENGTH_MAX];
} esp12f_cmd_easy_join;

typedef struct _esp12f_cmd_join
{
    uint8_t bssid[8]; // 6byte + 2byte PAD.
    char passwd[PASSWORD_LENGTH_MAX];
} esp12f_cmd_join;

typedef struct _esp12f_cmd_rssi
{
    uint8_t bssid[8]; // 6byte + 2byte PAD.
} esp12f_cmd_rssi;

typedef struct _esp12f_cmd_softap
{
    char ssid[SSID_NAME_LENGTH_MAX];
    char passwd[PASSWORD_LENGTH_MAX];
...
typedef struct _esp12f_resp_join
{
...
struct esp12f_cmd
{
    ...
    union
    {
        ...
        esp12f_cmd_join join;
        esp12f_cmd_rssi rssi;
...
struct esp12f_resp
{
    ...
    union
    {
        esp12f_resp_init init;
        esp12f_ap_info ap_info;
    } resp;
};

#define ESP12F_CMD_INIT              128
#define ESP12F_CMD_SCAN              129
#define ESP12F_CMD_JOIN              130
#define ESP12F_CMD_EASY_JOIN         131
#define ESP12F_CMD_RSSI              132
#define ESP12F_CMD_SOFTAP            133
...
/* porting */
//extern void usart_wifi_hw_init(void);
//extern void usart_wifi_int_cmd(rt_bool_t cmd);
//extern rt_bool_t usart_wifi_is_busy(void);

/* export API. */
extern rt_err_t rt_hw_wifi_init(const char *spi_device_name,wifi_mode_t mode);
extern int32_t esp12f_rssi(void);
extern rt_err_t esp12f_join(const char * SSID, const char * passwd);
extern rt_err_t esp12f_softap(const char * SSID, const char * passwd,uint32_t security,uint32_t channel);
