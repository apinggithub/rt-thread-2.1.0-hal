...
static rt_size_t _pin_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    ...
    if(status->status != PIN_TOGGLE)
        ...
        pin->ops->pin_toggle(dev, status->pin);
    return size;
}
...
FINSH_FUNCTION_EXPORT_ALIAS(rt_pin_mode, pinMode, set hardware pin mode<[PinNo] Mode[0->Out][1->In][2->PullUp][3->PullDown][4->Toggle]>);
...
FINSH_FUNCTION_EXPORT_ALIAS(rt_pin_write, pinWrite, write value to hardware pin<[PinNo] [1->High][0->Low]>);
...
FINSH_FUNCTION_EXPORT_ALIAS(rt_pin_read, pinRead, read status from hardware pin<[PinNo] [PinStatus]>);

void  rt_pin_toggle(rt_base_t pin)
{
	RT_ASSERT(_hw_pin.ops != RT_NULL);
    _hw_pin.ops->pin_toggle(&_hw_pin.parent, pin);
}
FINSH_FUNCTION_EXPORT_ALIAS(rt_pin_toggle, pinToggle, toggle to hardware pin<[PinNo]>);
